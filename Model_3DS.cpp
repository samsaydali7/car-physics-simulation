...
void Model_3DS::Load(char *name)
{
    ...
    fread(&main1.id,sizeof(main1.id),1,bin3ds);
    fread(&main1.len,sizeof(main1.len),1,bin3ds);

    // Start Processing
    MainChunkProcessor(main1.len, ftell(bin3ds));
    ...
    if (visible)
    {
        ...
        for (int i = 0; i < numObjects; i++)
        {
            ...
            for (int j = 0; j < Objects[i].numMatFaces; j ++)
            {
                ...
                //if(!textured){
                GLfloat mat_ambient[] = {Materials[Objects[i].MatFaces[j].MatIndex].ambient.r,Materials[Objects[i].MatFaces[j].MatIndex].ambient.g,Materials[Objects[i].MatFaces[j].MatIndex].ambient.b,1};
                GLfloat mat_diffuse[] = {Materials[Objects[i].MatFaces[j].MatIndex].diffuse.r,Materials[Objects[i].MatFaces[j].MatIndex].diffuse.g,Materials[Objects[i].MatFaces[j].MatIndex].diffuse.b,1};
                GLfloat mat_specular[] = {Materials[Objects[i].MatFaces[j].MatIndex].specular.r,Materials[Objects[i].MatFaces[j].MatIndex].specular.g,Materials[Objects[i].MatFaces[j].MatIndex].specular.b,1};
