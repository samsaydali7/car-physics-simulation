...
void GLTexture::LoadBMP(char *name)
{
    ...
    //TextureImage[0] = auxDIBImageLoad((char*)LPWSTR());
...
void GLTexture::LoadBMPResource(char *name)
{
    ...
    HRSRC hrsrc = FindResource(0, ptr, RT_BITMAP);
    ...
    unsigned char *ptr1 = (unsigned char *)buffer+sizeof(BITMAPINFO)+2;
	unsigned char temp;
...
void GLTexture::LoadTGAResource(char *name)
{
    ...
    wchar_t wtext2[20];
    mbstowcs(wtext2, "TGA", strlen("TGA")+1);//Plus null
    LPWSTR ptr2 = wtext2;
    HRSRC hrsrc = FindResource(0, ptr, ptr2);
